...
import fr.dao.app.External.jgraph.models.NExcel;
...
//        mHeightRatio = (mHCoordinate - 2 * mTextSize - mAbove - mTextMarging - mLinePointRadio - 5) / mSugHeightest;
    ...
    private void adjustWidth(int w) {
...
//        drawCoordinateAxes(canvas);
     ...
     * 画选择的 柱状图 数据
     */
    private void selectedText(Canvas canvas) {
...
//        textBg.close();
    ...
    private void drawSugExcel_text(Canvas canvas, NExcel excel, int i, boolean above) {
        ...
        //        if(i%2 == 0) {
        drawText(canvas, mFormat.format(excel.getUpper()) + excel.getUnit(), midPointF.x, (above ? (midPointF.y - textMarging) : (midPointF.y + mTextSize + mLinePointRadio)) * ratio, (above ? (excel.getRectF().bottom - (excel.getHeight() + textMarging) * ratio) : (excel
                .getRectF().bottom - (excel.getHeight() - mTextSize - mLinePointRadio) * ratio)));

        //柱子 底部文字
        if (excel.getLower() > 0 && mChartStyle == ChartStyle.BAR) {
            drawText(canvas, mFormat.format(excel.getLower()) + excel.getUnit(), midPointF.x, (excel.getStart().y + textMarging + mTextSize) * ratio, excel.getStart().y + excel.getLower() - (excel.getLower() - mTextSize) * ratio);
        }
    }

    private void drawText(Canvas canvas, String text, float x, float y, float y2) {
...
//            canvas.drawPoint(midPointF.x, midPointF.y, mLinePaint);
    ...
    private void gradientUnderline(Canvas canvas) {
