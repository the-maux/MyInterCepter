    ...
    private void                        useJsoup(final String site) throws IOException {
        new Thread(new Runnable() {
            public void run() {
                Document doc = null;
                try {
                    doc = Jsoup.connect(URL + site).get();
                    Elements elems = doc.getElementsByClass("row");
                    /**
                     * 1er ROW: <h1> [HTTPS] blog.valvin.fr <span class="small">(Tue, 31 Jul 2018 10:53:33 +0000)</span> </h1>
                     *         + contient HOSTNAME + Lastdate checked
                     *         +
                     *
                     * */
                    int a = 0;
                    for (Element elem : elems) {
                        Log.d(TAG, "---------------------------------------------------------------");
                        Log.d(TAG, elem.outerHtml());
//                        for (Element element : elem.getAllElements()) {
//                            Log.d(TAG, "\t\t" + elem.outerHtml());
//                        }
                    }
                } catch (IOException e) {
...
/***
 * view-source:https://cryptcheck.fr/https/blog.valvin.fr
 *
 D/Cryptcheck: ---------------------------------------------------------------
 <div class="row">
 <div class="col-sm-11">
 ...
 <div class="row">
 <div class="col-sm-12">
 <h2> <span class="label label-state-warning">E</span> 51.15.51.99 : 443 <span class="small">(blog.valvin.fr)</span></h2>
 </div>
 </div>
 ---------------------------------------------------------------
 <div class="row">
