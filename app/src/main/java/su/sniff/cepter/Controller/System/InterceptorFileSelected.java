...
public class                    InterceptorFileSelected implements OpenFileDialog.OnFileSelectedListener {
    ...
    public void                 onFileSelected(File f) {
        ...
        File fDroidSheep = new File(Singleton.getInstance().FilesPath + "/exits.id");
        if (fDroidSheep.exists()) {
            fDroidSheep.delete();
        }
        activity.sniff_process = new RootProcess("InterceptorFileSelected", Singleton.getInstance().FilesPath);
        activity.sniff_process.exec(Singleton.getInstance().FilesPath + "/cepter " + f.getAbsolutePath() + " " + "1");//1 pour la resurection? i dunno what this mean
